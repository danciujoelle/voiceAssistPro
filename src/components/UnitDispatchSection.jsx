  ...
  useEffect(() => {
    ...
    const stationIconSvg = `
      <svg width="32" height="32" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x="16" y="20" text-anchor="middle" fill="white" font-size="16">${getUnitIcon(
          unit
        )}</text>
      </svg>
    `;
...
// Accepts an optional dispatchEventId prop to uniquely identify each dispatch trigger
const UnitDispatchSection = ({
  emergencyData,
  incidentCoordinates,
  isDispatchTriggered,
  dispatchEventId, // new prop
}) => {
  ...
  // Set hasRendered to true after section is mounted
  useEffect(() => {
  ...
  }, [isDispatchTriggered, dispatchedUnits, incidentCoordinates]);

  // Scroll after section is rendered, also when dispatchEventId changes
  useEffect(() => {
    if (hasRendered && sectionRef.current) {
      sectionRef.current.scrollIntoView({ behavior: "smooth", block: "start" });
    }
    // eslint-disable-next-line
  }, [hasRendered, dispatchEventId]);

  if (!incidentCoordinates || !dispatchedUnits.length) {
    return null;
  }

  return (
    <GoogleMapsWrapper>
      <section className="unit-dispatch-section" ref={sectionRef}>
...
UnitDispatchSection.propTypes = {
  ...
  dispatchEventId: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
};
